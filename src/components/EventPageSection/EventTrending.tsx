      ...
      >
        ...
        <div className="absolute inset-0 p-5 flex flex-col justify-end bg-gradient-to-t from-black via-transparent to-transparent">
          ...
          <div className="flex items-center">
            {category && (
              <Tag text={category} className="mt-4 bg-gray-blue capitalize" />
            )}
          </div>
          <div className="mt-4 space-y-1">
            <p className="text-lg text-white">
              <span className="font-bold">Registration Ends:</span>{" "}
              {moment(registrationEndDate).zone(timezone).format("DD MMM YYYY")}
            </p>
            <p className="text-lg text-white">
              <span className="font-bold">Event Starts:</span>{" "}
              {moment(eventStartDate).zone(timezone).format("DD MMM YYYY")}
            </p>
          </div>
        </div>
      </motion.div>
